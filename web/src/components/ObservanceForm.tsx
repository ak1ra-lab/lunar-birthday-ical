        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          {!initialData && (
              <div className="space-y-2 p-3 bg-slate-50 rounded-md border text-sm">
                  ...
                  >
                      ...
                      {PRESETS.map(p => (
                          <option key={p.key} value={p.key}>{t(`holidays.${p.key}`)}</option>
                      ))}
                  </select>
              </div>
          )}
          ...
          <div className="grid grid-cols-3 gap-4">
            <div className="space-y-2">
                ...
                <select {...register('week')} className="flex h-10 w-full rounded-md border border-slate-200 bg-white px-3 py-2 text-sm">
                    {[1, 2, 3, 4].map(w => <option key={w} value={w}>{t(`observanceForm.weeks.${w}`)}</option>)}
                </select>
            </div>
            <div className="space-y-2">
                <Label>{t('observanceForm.weekday')}</Label>
                <select {...register('weekday')} className="flex h-10 w-full rounded-md border border-slate-200 bg-white px-3 py-2 text-sm">
                    {[0, 1, 2, 3, 4, 5, 6].map(w => <option key={w} value={w}>{t(`observanceForm.weekdays.${w}`)}</option>)}
                </select>
            </div>
            <div className="space-y-2">
                <Label>{t('observanceForm.month')}</Label>
                <select {...register('month')} className="flex h-10 w-full rounded-md border border-slate-200 bg-white px-3 py-2 text-sm">
                    {Array.from({length: 12}, (_, i) => i + 1).map(m => <option key={m} value={m}>{t(`observanceForm.months.${m}`)}</option>)}
                </select>
            </div>
          </div>
